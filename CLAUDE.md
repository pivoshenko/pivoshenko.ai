...
- `rules/` — mirror copies for versioning; the live files win. `CLAUDE.md` mirrors `~/.claude/CLAUDE.md` (the canonical global CLAUDE.md, a real file); `VAULT.md` mirrors the Obsidian vault's `CLAUDE.md`. Refresh after editing a canonical file; `wiki-lint` checks both for drift.
