...
- `skills/` — Locally authored skills. Each subdir contains a required `SKILL.md` (frontmatter: `name`, `description`, optional `tags: [...]`, optional `updated_at`) and optional `references/`, `scripts/`, `assets/`.
  ...
  - `wiki-capture`, `wiki-summarize`, `wiki-project`, `wiki-write`, `wiki-lint` — Obsidian second-brain workflows over the vault at `~/Vault` (symlink → iCloud path, created by `just vault-link`; schema lives in the vault's own `CLAUDE.md`; skills hold workflow only, zero duplicated conventions). Tags: `wiki`, `obsidian`.
