...
>
> **After:** The company's founding date is not documented in the available sources. (Or cut the sentence. State a date only if a source provides one.)
...
Say what isn't known, or cut. Never dress a guess up as fact.
...
Em/en dashes are the highest-signal tell; treat removal as a hard constraint, not a preference. Replacement order: period > comma > colon > parentheses > restructure. Catch spaced ` — ` and double ` -- ` too.
...
Same pass for the rest of the ASCII rule:

> **Before:** He said “the project is on track”… then reversed course → chaos.
>
> **After:** He said "the project is on track"... then reversed course -> chaos.
