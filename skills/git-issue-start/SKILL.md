...
description: >-
  Take a GitHub issue from open to a checked-out branch - assign it to you, mark it in progress if the
  repo has such a label, derive a conventional branch name from its `type: *` label, branch off the
  detected base. Use for "start issue 42", "work on #42", "/git-issue-start 42", "let's do 42", "pick
  up that issue", "take the oauth issue", "I'll take this one", or whenever the user points at a
  tracked issue and is about to write code for it. Resolves an issue named by title via `gh issue list
  --search`. Boundary: with no issue in play it is `git-branch-create`'s, and this skill delegates
  naming and base detection to it. Assigns and branches immediately without asking for confirmation.
tags: [git, github]
...
7. Name and base follow `git-branch-create` exactly - its **Naming** section and its base-detection incantation. Do not restate either. The desc is the issue title, kebab-cased, verb made imperative present, with the whole `<type>/<desc>` name 60 chars or fewer; over -> cut per that skill's desc rules, never truncate mid-word
8. Name conflict check, before the tracker changes: `git show-ref --verify --quiet refs/heads/<name>` succeeds -> the branch already exists; stop, surface the conflict, no overwrite. Why -> checking after assignment would halt the run with the issue assigned and labelled in progress but no branch created - the exact half-state the Rules forbid
9. Assign to self: `gh issue edit <n> --add-assignee @me`
10. In-progress label, only if one already exists. Fetch the repo's labels once per run per canon label resolution:
    ```bash
...
11. Create + checkout per `git-branch-create` step 5
12. Record the issue on the branch:
    ```bash
    git config branch.<name>.issue <n>
    ```
    Why -> the branch name carries no ticket id, and `git-commit` reads this key to write the `Closes #<n>` trailer. Skip it and the commit has no link back to the tracker. Not `gh issue develop` - the siblings read `branch.<name>.issue`, and develop's GitHub-side link carries no local key for `git-commit` and `git-pr-create` to read
13. Print the report
...
`git-branch-create` keeps ticket IDs out of branch names unless asked, and starting from an issue does not change that: the link to `#n` rides in `branch.<name>.issue` (step 12), the commit footer (`Closes #42`), and the PR body (`Resolves: #42`) - all three of which a tool actually reads. A branch name is a human label, not a foreign key.
